...
export const AppProvider = ({ children }) => {
  ...
  const saveTaskTimerLog = (taskId, customNotes = '') => {
    ...
    const newLog = {
      ...
      hourlyRate: assignee ? (assignee.hourlyRate || 10500) : (proj ? (proj.hourlyRate || 10500) : 10500),
      billable: true,
      invoiced: false,
  ...
  const addTimeLog = (logData) => {
    ...
    const newLog = {
      id: `log_${Date.now()}`,
      projectName: proj ? proj.name : 'Project',
      service: proj ? proj.service : 'Web Development',
      hourlyRate: proj ? (proj.hourlyRate || 10500) : 10500,
      billable: true,
      invoiced: false,
      date: new Date().toISOString().split('T')[0],
      ...logData
