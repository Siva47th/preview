...
import { Search, Plus, Bell, Shield, Sparkles, Clock, CheckCircle2, LogOut, Database, UserCheck, Settings } from 'lucide-react';
    ...
    <header className="h-16 bg-white border-b border-slate-200 px-6 flex items-center justify-between sticky top-0 z-30 shadow-sm">
      ...
      <div className="flex items-center gap-4">
        ...
        <button
          ...
          className="px-3 py-1.5 rounded-lg bg-slate-100 hover:bg-slate-200 text-slate-700 relative transition flex items-center gap-1.5 text-xs font-semibold border border-slate-200"
          title="Deployment & Storage Manager"
        >
          <Database className="w-3.5 h-3.5 text-indigo-600" />
          <span className="hidden xl:inline text-[11px]">Storage & Cloud</span>
        </button>
        ...
        <div className="flex items-center gap-3 pl-3 border-l border-slate-200">
          <button
            ...
            className="flex items-center gap-2 p-1.5 rounded-xl hover:bg-slate-100 transition group text-left"
            title="Edit My Custom Profile"
          >
            <div className="relative">
              <img src={currentUser.avatar} alt={currentUser.name} className="w-8 h-8 rounded-full object-cover border border-slate-300 shrink-0 group-hover:border-indigo-600 transition" />
